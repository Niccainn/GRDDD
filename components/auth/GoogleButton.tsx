/**
 * "Continue with Google" button + "or" divider. Both hide together when
 * Google OAuth is not configured, so the password form stands alone
 * without a dangling divider.
 *
 * The link is a plain anchor (not a fetch) because OAuth requires a
 * top-level browser navigation — we need the user agent to follow the
 * redirect chain through Google and back to /api/auth/oauth/google/callback.
 */
'use client';
